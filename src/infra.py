...
class AccountInMemoryRepo(AccountGetter, AccountSaver):
    ...
    def save(self, account: Account):
        exists = next(filter(lambda elm: elm.id() == account.id(), self.data), None)
        if exists is not None:
            self.data = [account if elem.id() == elem.id() else elem for elem in self.data]
        else:
...
class AccountMongoRepo(AccountGetter, AccountSaver):
    ...
    def save(self, account: Account):
        self._accounts.update({'user_id': account.id()}, account.to_dict(), upsert=True)
